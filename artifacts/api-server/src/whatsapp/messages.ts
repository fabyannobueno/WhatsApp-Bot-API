...
export function getMessage(path: string, replacements: Record<string, string> = {}): string {
  ...
  let message: unknown = botMessages;

  for (const part of parts) {
    if (message && typeof message === 'object' && part in (message as Record<string, unknown>)) {
  ...
  if (message && typeof message === 'object' && 'message' in (message as Record<string, unknown>)) {
    let result = (message as { message: string }).message;
    for (const [key, value] of Object.entries(replacements)) {
      result = result.replace(`{${key}}`, value);
    }
    return result;
